...
class LogRegression:
    ...
    def __init__(self, b_bias=0, b_p1elo=0, b_p2elo=0, b_p1winsvp2=0, b_p2winsvp1=0, b_p1winpct=0, b_p2winpct=0):
        self.b_bias = b_bias
        ...
        for i in range(epochs):
            ...
            for fight in training_data:
                ...
                if prediction > 0.5 and fight['winner'] == 2:
                    correct += 1
                self.b_bias = self.recalc_beta(self.b_bias, fight['winner']-1, prediction, 1) # bias always has coefficient 1
                self.b_p1elo = self.recalc_beta(self.b_p1elo, fight['winner']-1, prediction, fight['p1elo'])
                self.b_p2elo = self.recalc_beta(self.b_p2elo, fight['winner']-1, prediction, fight['p2elo'])
                self.b_p1winsvp2 = self.recalc_beta(self.b_p1winsvp2, fight['winner']-1, prediction, fight['p1winsvp2'])
                self.b_p2winsvp1 = self.recalc_beta(self.b_p2winsvp1, fight['winner']-1, prediction, fight['p2winsvp1'])
                self.b_p1winpct = self.recalc_beta(self.b_p1winpct, fight['winner']-1, prediction, fight['p1winpct'])
                self.b_p2winpct = self.recalc_beta(self.b_p2winpct, fight['winner']-1, prediction, fight['p2winpct'])
            self.log_betas()
    ...
    def recalc_beta(self, b, y, prediction, x):
        return b + self._ALPHA * (y-prediction) * prediction * (1-prediction) * x
