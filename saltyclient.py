...
class SaltyClient():
    ...
    def spoof_login(self, spoof_cookie):
        ...
        try:
            ...
            self.log.info("Spoof'd cookie %s " % cookie)
            self.log.debug('headers: %s' % self.session.headers)
            self.log.debug('cookies: %s' % self.session.cookies)
        except IndexError as e:
            self.log.error('Invalid cookie format. Please use format "id0=foo; id1=bar". Continuing without spoof.')
    ...
    def login(self, email, password):
        ...
        self.spoof_enabled = False
        self.log.debug('response: %s' % response.text.strip()[:20], response.headers)
        self.log.debug('headers: %s' % self.session.headers)
        self.log.debug('cookies: %s' % self.session.cookies)
        self.log.info('Logged in as %s' % format(email))
    ...
    def get_wallet_balance(self):
        ...
        logging.info('headers' + str(self.session.headers))
        logging.info('resp headers' + str(page_response.headers))
